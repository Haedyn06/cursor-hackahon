...
export function InterviewPrepPreviewPanel({
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden bg-[var(--background)]">
      <div className="flex shrink-0 items-center justify-between gap-5 border-b-[2.5px] border-[var(--foreground)] bg-white px-10 py-5">
        <div>
          <button
            type="button"
            onClick={onBack}
          ...
          <h1 className="font-heading text-[20px] font-extrabold">{prep.title}</h1>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <div className="flex shrink-0 flex-wrap gap-1.5 border-b-2.5 border-[var(--foreground)] bg-white px-10 pt-5 pb-0">
        ...
        <div className="flex flex-1 flex-col gap-2.5 overflow-y-auto p-10">
          <div className="mx-auto w-full max-w-[820px]">
            {activeQuestions.map((item, i) => (
              <div
                key={i}
                className="mb-2.5 overflow-hidden rounded-2.5xl bg-white neo-border"
              >
                <button
                  type="button"
                  onClick={() => setExpanded((prev) => ({ ...prev, [i]: !prev[i] }))}
                  className="flex w-full cursor-pointer items-center justify-between gap-3 border-none bg-transparent px-5 py-5 text-left"
                >
                  <span className="text-sm font-bold">{item.q}</span>
                  <span className="shrink-0 text-[#888]">{expanded[i] ? "▲" : "▼"}</span>
                </button>
                {expanded[i] && (
                  <div className="border-t-2 border-[var(--foreground)] px-5 pt-2.5 pb-5">
                    <NeoBadge color="var(--lav)" className="mb-2.5 text-[10px]">
                      Answer framework
                    </NeoBadge>
                    <p className="text-[10px] leading-relaxed font-medium text-[#444]">
                      {item.a}
                    </p>
                  </div>
                )}
